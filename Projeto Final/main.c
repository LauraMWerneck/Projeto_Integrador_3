...
#define ULTRASONIC_TRIG_PIN GPIO_NUM_19 // Pino TRIG do sensor ultrassônico
#define ULTRASONIC_ECHO_PIN GPIO_NUM_18 // Pino ECHO do sensor ultrassônico
#define RELE_PIN GPIO_NUM_23            // Pino do relé
...
// OBS: é preciso dar um ping e ver o endereço do IPV4 e colocar ele como o IP antes de :50000/ESP_data
...
void send_data(float vazao, float volume_total, float distancia, float volume_por_minuto, bool falta_agua) {
    char payload[256]; // Buffer para armazenar os dados em formato JSON
    snprintf(payload, sizeof(payload), "{\"flow_rate\":%.2f,\"total_volume\":%.2f,\"distance\":%.2f,\"volume_minuto\":%.2f, \"falta_agua\":%d}", vazao, volume_total, distancia, volume_por_minuto, falta_agua);
...
// Configuração do GPIO do relé
void setup_relay() {
    gpio_set_direction(RELE_PIN, GPIO_MODE_OUTPUT); // Configura o pino como saída
    gpio_set_level(RELE_PIN, 0); // Começa desligado
}
...
void sensor_task(void *pvParameters) {
    ...
    while (1) {
        ...
        float distancia = measure_distance(); // Mede a distância
        ...
        // Se a distância for menor ou igual a 5 cm, liga o relé
        if (distancia > 0 && distancia <= 5) {

            gpio_set_level(RELE_PIN, 1);
            ESP_LOGI(TAG, "Relé ligado!");
            falta_agua = 0;
            ...
            if (vazao == 0){
                falta_agua = 1;
                ESP_LOGI(TAG, "ALERTA: Falta de água detectada!");
            }else{
                falta_agua = 0;
                ESP_LOGI(TAG, "Tem água!");  
            }
        }

        ESP_LOGI(TAG, "Vazão: %.2f L/min, Volume Total: %.2f L, Distância: %.2f cm, Falta água: %d", vazao, volume_total, distancia, falta_agua);
        ...
        if (segundos >= 60) { // Após 60 segundos
            ...
            send_data(vazao, volume_total, distancia, volume_por_minuto, falta_agua);  
