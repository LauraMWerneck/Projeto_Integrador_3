...
void flow_task(void *pvParameters) {
    ...
    const float intervalo_segundos = 1.0;         // Intervalo de cálculo (1 segundo)

    while (1) {
        uint32_t pulso_inicial = contador_pulsos; // Captura os pulsos atuais
        contador_pulsos = 0;                      // Reseta o contador
        vTaskDelay(pdMS_TO_TICKS(intervalo_segundos * 1000)); // Delay de 1 segundo

        float vazao_litros_por_minuto = (pulso_inicial / intervalo_segundos) * (60.0 / PULSOS_POR_LITRO);
        float volume = vazao_litros_por_minuto / 60; // Volume em litros no intervalo de 1 segundo
        volume_total += volume;                     // Atualiza o volume total
